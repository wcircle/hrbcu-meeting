...
<body>
    <div class="page">
        ...
        <div class="index_bd">
            ...
            <div class="i_col_r">
                <div class="location"><b class="bg_i ico"></b>您的位置：<a href="/backAdmin/eat.html">就餐管理</a> </div>
                <div class=" block2 sub_ct">
                    <form id="search-form" class="text-l form form-horizontal" method="post" action="/backAdmin/getEatList">
                        ...
                        <div class="row cl">
                            ...
                            <div class="formControls col-xs-2 col-sm-2 size-S pr5 pl5">
                                <input class="input-text radius size-S" type="text" name="userName" value="${eatSearch.userName}" placeholder="姓名" />
                            </div>
                            ...
                            <div class="formControls col-xs-3 col-sm-3 size-S pr5 pl5">
                                <select id="representCollege" name="representCollege" class="select-box radius size-S">
                                    ...
                                    <c:forEach items="${collegeList}" var="college">
                                        <option value="${college.name}" <c:if test="${college.name == eatSearch.representCollege}">selected</c:if>>${college.name}</option>
                                    </c:forEach>
                                </select>
                            </div>
                            <div class="form-label col-xs-1 col-sm-1 size-S pr5 pl5">就餐地点</div>
                            <div class="formControls col-xs-3 col-sm-3 size-S pr5 pl5">
                                <input class="input-text radius size-S" type="text" name="address" value="${eatSearch.address}" placeholder="就餐地点" />
                            </div>
                        </div>
                        <div class="row cl">
                            <div class="form-label col-xs-1 col-sm-1 size-S pr5 pl5" >房间号</div>
                            <div class="formControls col-xs-2 col-sm-2 size-S pr5 pl5">
                                <input class="input-text radius size-S" type="text" name="roomNo" value="${eatSearch.roomNo}" placeholder="房间号" />
                            </div>
                            <div class="form-label col-xs-1 col-sm-1 size-S pr5 pl5">桌号</div>
                            <div class="formControls col-xs-3 col-sm-3 size-S pr5 pl5">
                                <input class="input-text radius size-S" type="text" name="tableNo" value="${eatSearch.tableNo}" placeholder="桌号" />
                            </div>
                            <div class="form-label col-xs-1 col-sm-1 size-S pr5 pl5">就餐序号</div>
                            <div class="formControls col-xs-2 col-sm-2 size-S pr5 pl5">
                                第<input class="input-text radius size-S" type="number" name="eatOrder" value="${eatSearch.eatOrder}" placeholder="就餐序号"/>餐
                            </div>
                    ...
                    <table class="table table-border table-bordered table-hover table-bg table-sort mt-20">
                        ...
                        <tr>
                            ...
                            <span class="l" style="margin-right: 10px;">
                                <a href="javascript:eatAdd()" style="color: white" data-title="添加就餐" class="btn btn-primary size-S radius" >
                                    <i class="Hui-iconfont" style="line-height: 20px;">&#xe600;</i> 添加就餐
                                </a>
                            </span>
                            <span class="l" style="margin-right: 10px">
                                <a href="javascript:void(0);" onclick="deleteEat()" class="btn btn-danger size-S radius" style="color: white">
                                    <i class="Hui-iconfont" style="line-height: 20px;">&#xe6e2;</i> 批量删除
                                </a>
                            </span>
                            <span class="l">
                                <a href="javascript:void(0);" onclick="exportExcel()" class="btn btn-warning size-S radius" style="color: white">
                                    <i class="Hui-iconfont" style="line-height: 20px;">&#xe644;</i> 批量导出
                                </a>
                            </span>
                            <a class="btn btn-success radius r" style="line-height:1.6em;color: white" href="javascript:location.replace('/backAdmin/eat.html');" title="刷新"><i class="Hui-iconfont">&#xe68f;</i></a>
                            </th>
                        </tr>
                        <tr class="text-c">
                        ...
                        <tbody>
                        <c:forEach items="${eatList}" var="eat" varStatus="status">
                            <tr class="text-c">
                                <td><input type="checkbox" name="ids" value="${eat.eatId}"></td>
                                <td>${status.index+1}</td>
                                <td>${eat.userName}</td>
                                <td>${eat.phone}</td>
                                <td>第${eat.eatOrder}餐</td>
                                <td>${eat.address}</td>
                                <td>${eat.roomNo}</td>
                                <td>${eat.tableNo}</td>
                                <td>
                                    <a title="修改" href="javascript:eatEdit('${eat.eatId}')" data-title="修改" class="ml-5" style="text-decoration:none">
    ...
    <script type="text/javascript">
                ...
                $.ajax({
                    ...
                    success: function(msg) {
                        ...
                        if(data.status == 0){
                            ...
                            setTimeout(function () {
                                location.replace("/backAdmin/eat.html");
                            },2000);
        ...
        function eatEdit(id) {
            layer.open({
                ...
                shade: 0.8,
                area: ['70%', '80%'],
                content: '/backAdmin/eatEdit.html/'+id
            });
        }
